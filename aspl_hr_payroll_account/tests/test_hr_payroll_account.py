...
class TestHrPayrollAccount(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.payslip_action_id = self.ref('aspl_hr_payroll.menu_department_tree')
        ...
        self.hr_structure_softwaredeveloper = self.env['hr.payroll.structure'].create({
            ...
            'parent_id': self.ref('aspl_hr_payroll.structure_base'),
            'rule_ids': [(6, 0, [
                    self.ref('aspl_hr_payroll.hr_salary_rule_houserentallowance1'),
                    self.ref('aspl_hr_payroll.hr_salary_rule_convanceallowance1'),
                    self.ref('aspl_hr_payroll.hr_salary_rule_professionaltax1'),
                    self.ref('aspl_hr_payroll.hr_salary_rule_providentfund1'),
                    self.ref('aspl_hr_payroll.hr_salary_rule_meal_voucher'),
                    self.ref('aspl_hr_payroll.hr_salary_rule_sales_commission')
            ])],
        })
        ...
        self.hr_contract_john = self.env['hr.contract'].create({
            ...
            'journal_id': self.ref('aspl_hr_payroll_account.expenses_journal'),
        })
