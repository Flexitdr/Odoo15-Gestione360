...
class HrPayslip(models.Model):
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            ...
            # compute worked days
            work_data = contract.employee_id._get_work_days_data_batch(day_from, day_to, calendar=contract.resource_calendar_id)[contract.employee_id.id]
            attendances = {
    ...
    def onchange_employee_id(self, date_from, date_to, employee_id=False, contract_id=False):
        ...
        self.get_worked_day_lines(contracts, date_from, date_to)
    ...
    def onchange_employee(self):
        ...
        if not self.env.context.get('contract') or not self.contract_id:
            ...
            for worked_days_line_id in worked_days_line_ids:
                self.worked_days_line_ids = [(0,0,worked_days_line_id)]
    
            input_line_ids = self.get_inputs(contracts, date_from, date_to)
            for input_line_id in input_line_ids:
                self.input_line_ids = [(0,0,input_line_id)]
