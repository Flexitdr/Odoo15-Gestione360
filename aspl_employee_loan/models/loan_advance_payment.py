...
class LoanAdvancePayment(models.Model):
    ...
    def preview_payments(self):
        for record in self:
            ...
            if record.loan_app_id and record.amount:
                loan_payment_ids = loan_payment.search([('loan_app_id', '=', record.loan_app_id.id),
                                                            ('state', '=', 'draft')])
                loan_payment_paid_ids = loan_payment.search([('loan_app_id', '=', record.loan_app_id.id),
                                                                    ('state', '=', 'paid')])
                balance_list = []
                for each in loan_payment_paid_ids:
                    balance_list.append(each.balance_amt)
                if not balance_list:
                    remain_balance = record.loan_app_id.amount
                if balance_list:
                    remain_balance = balance_list[-1]
                bal = "%.2f" % (remain_balance)
                if record.amount and record.amount > float(bal):
                    raise ValidationError(_('Please enter the amount less than %s' % (bal)))
                bal = "%.2f" % (remain_balance)
                if record.amount == float(bal):
                    final_list = []
                else:
                    if record.loan_app_id.loan_method == 'reducing':
                        principal = remain_balance - record.amount
                        months = len(loan_payment_ids)
                        rate = record.loan_app_id.loan_type_id.interest_rate / 100.00
                        ...
                        if record.loan_app_id.loan_type_id and record.loan_app_id.loan_type_id.interest_rate:
        ...
        #                             if self.loan_app_id.loan_type_id and self.loan_app_id.loan_type_id.interest_rate:
                        ...
                        elif record.loan_app_id.loan_type_id and not record.loan_app_id.loan_type_id.interest_rate:
                            principal = remain_balance - record.amount
                            rate = record.loan_app_id.loan_type_id.interest_rate / 100 * principal
                            time = float(len(loan_payment_ids)) / 12
                            months = record.loan_app_id.term
                            per = np.arange(months) + 1
                            each_month_payment = balance = 0.00
                            if time:
                                balance = ((principal / time + rate) / 12) * record.loan_app_id.term
                                for each_term in per:
                                    date = datetime.date.today() + relativedelta(months=each_term)
                                    date = date.replace(day=1)
                                    if record.loan_app_id.loan_type_id and record.loan_app_id.loan_method:
                                        interest = principal / time + rate
                                        each_month_payment = interest / 12
                                        total_pay_amount = each_month_payment * record.loan_app_id.term
                                        balance -= each_month_payment
                                        monthly_interest = rate * time / record.loan_app_id.term
                                        monthly_principal = principal / record.loan_app_id.term
                                        final_list.append((0, 0, {
                                                        'due_date': date,
                                                        'number': each_term,
                                                        'principal': monthly_principal,
                                                        'interest': monthly_interest,
                                                        'interest_rate': str("%.2f" % (record.loan_app_id.loan_type_id.interest_rate / 12)) + " %",
                                                        'total': monthly_interest + monthly_principal,
                                                        'balance_amt': abs(balance)
                                                        }))
                    elif record.loan_app_id.loan_method == 'flat':
                        principal = remain_balance - record.amount
                        rate = record.loan_app_id.loan_type_id.interest_rate / 100 * principal
                        time = float(len(loan_payment_ids)) / 12
                        months = record.loan_app_id.term
            ...
            record.lap_line_ids = final_list
            return final_list
...
#                 bal = "%.2f" % (remain_balance)
        ...
        template_id = self.env.ref('aspl_employee_loan.email_template_for_new_loan_emi')
...
class LoanAdvancePaymentLine(models.Model):
