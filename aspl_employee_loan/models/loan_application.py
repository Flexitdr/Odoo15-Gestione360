...
class LoanApplication(models.Model):
    ...
    def rejected_mail_template(self):
        template_id = self.env.ref('aspl_employee_loan.email_template_for_reject_loan')
    ...
    def cancelled_mail_template(self):
        template_id = self.env.ref('aspl_employee_loan.email_template_for_cancel_loan')
        template_id.send_mail(self.id, force_send=True)
    ...
    def rejected(self):
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': self.env.ref('aspl_employee_loan.wizard_loan_form_view').id,
    ...
    def verified_approve(self):
        ...
        loan_id = self.search(
            [('employee_id', '=', self.employee_id.id), ('id', '!=', self.id), ('state', '!=', 'close')])
        ...
        template_id = self.env.ref('aspl_employee_loan.email_template_for_loan_manager')
    ...
    def button_approved(self):
        template_id = self.env.ref('aspl_employee_loan.email_template_for_apporve_loan')
        template_id.send_mail(self.id, force_send=True)
    ...
    def button_paid(self):
        ...
        move_id = self.env['account.move'].create(
            {'journal_id': self.account_journal_id.id, 'ref': self.loan_id, 'line_ids': move_line})
        move_id.post()
        ...
        for each in self.loan_payment_ids:
            if count <= len_date:
                if lst_date:
                    ...
                    count += 1
        template_id = self.env.ref('aspl_employee_loan.email_template_for_paid_loan')
        template_id.send_mail(self.id, force_send=True)
    ...
    @api.depends('loan_payment_ids.principal', 'loan_payment_ids.interest', 'loan_payment_ids.total',
                 'loan_payment_ids.state', 'loan_payment_ids.extra')
        ...
        for payment in self:
            ...
            if payment.currency_id:
                payment.update({'total_paid_amt': payment.currency_id.round(paid_amt + pre_payment_amount),
                                'total_remaining_amt': payment.currency_id.round(
                                    payment.amount) - payment.currency_id.round(paid_amt + pre_payment_amount),
    ...
    def unlink(self):
        ...
        return super(LoanApplication, self).unlink()
    ...
    def loan_user_url(self):
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        action = self.env.ref('aspl_employee_loan.action_loan_application').id
        view_type = 'form'
        menu_id = self.env.ref('aspl_employee_loan.menu_loan_application_draft').id
        return base_url + "/web#id=" + str(self.id) + "&action=" + str(
            action) + "&model=" + 'loan.application' + "&view_type=" + view_type + "&menu_id=" + str(menu_id)
    ...
    def loan_close(self):
        ...
        template_obj = self.env.ref('aspl_employee_loan.email_template_loan_close')
        template_obj.send_mail(self.id, force_send=True)

    @api.depends('loan_payment_ids.state')
    def _check_loan_fully_paid(self):
        for record in self:
            if record.loan_payment_ids:
                all_record = all([x.state == 'paid' for x in record.loan_payment_ids if record.loan_payment_ids])
                if all_record:
                    record.fully_paid = True

    @api.model
    def default_get(self, fields):
        res = super(LoanApplication, self).default_get(fields)
        loan_setting_id = self.env['loan.setting'].sudo().search([], limit=1, order='id desc')
    ...
    def create_loan_calc(self):
        ...
        return {
            ...
            'view_id': self.env.ref('aspl_employee_loan.loan_calc_form').id,
            'target': 'new'
        }
...
class ApplicantCategory(models.Model):
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        ...
        return super(ApplicantCategory, self).name_search(name, args)
